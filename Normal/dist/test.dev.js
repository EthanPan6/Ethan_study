...
var collectionCopy = JSON.parse(JSON.stringify(collection)); // 请把你的代码写在这条注释以下
...
var squaredIntegers = squareList(realNumberArray); // console.log(squaredIntegers);
////////////////////////////////////////////////////////
...
function makeList(arr) {
  ...
  var resultDisplayArray = arr.map(function (item) {
...
var resultDisplayArray = makeList(result.failure); // console.log(resultDisplayArray)
...
var results = getNine; //   console.log(results());
///////////////////////////////

function mixedNumbers(arr) {
  // 请把你的代码写在这条注释以下
  var header = ['I', 2, 'three'];
  var footer = [7, 'VIII', 9];
  arr.unshift.apply(arr, header);
  arr.push.apply(arr, footer); // 请把你的代码写在这条注释以上
...
function copyMachine(arr, num) {
  ...
  while (num >= 1) {
    // 请把你的代码写在这条注释以下
    newArr = [].concat(_toConsumableArray(newArr), [arr]); // 请把你的代码写在这条注释以上
...
function countOnline(obj) {
  ...
  for (var item in obj) {
    ...
    count += obj[item].online; // console.log(item.hasOwnProperty("online"))
  }

  return count; // 请把你的代码写在这条注释以上
...
function findLongestWord(str) {
  ...
  var array = str.split(''); // 得到数组 ["May", "the", "force", "be", "with", "you"]

  var longest = 0; // 第2步：对数组array做遍历，并且将符合条件的值赋值给longest
...
function confirmEnding(str, target) {
  ...
  // -- Falcor
  return str.substring(str.length - target.length) == target; // return str.substring(0 - target.length) == target;
...
function frankenSplice(arr1, arr2, n) {
  var newArr1 = _toConsumableArray(arr1);

  var newArr2 = _toConsumableArray(arr2);

  newArr2.splice.apply(newArr2, [n, 0].concat(_toConsumableArray(newArr1)));
  return newArr2;
...
function mutation0(arr) {
  ...
  var str1Arr = arr[1].toLowerCase().split("");
  var len = str1Arr.length;
  var isHere = true;
...
var duck = new Bird(); // duck.eat(); // 输出 "nom nom nom"
// duck.fly(); // 输出 "I'm flying!"
///////////////////

var funModule = function () {
  return {
    isCuteMixin: function isCuteMixin(obj) {
      obj.isCute = function () {
        return true;
      };
    },
    singMixin: function singMixin(obj) {
      obj.sing = function () {
...
function remove(list, bookName) {
  ...
  if (list.indexOf(bookName) >= 0) {
    newList.splice(list.indexOf(bookName), 1); // 请在本行以上添加你的代码
  }
...
var newerBookList = remove(bookList, 'On The Electrodynamics of Moving Bodies'); // var newestBookList = remove(add(bookList, 'A Brief History of Time'), 'On The Electrodynamics of Moving Bodies');
...
Array.prototype.myMap = function (fn) {
  var thisValue = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : _this;
  console.log(thisValue);
  var arr = [];

  for (var index in _this) {
    // 执行回调函数时绑定this，并将结果push进新数组
    arr.push(fn.call(thisValue, _this[index], index, _this));
  } // 返回新数组
...
(function () {
  ...
  console.log(arr.myMap(function (i) {
    return i + 1;
  }));
})();