...
注意:与变量不同是this没有作用域的限制，嵌套函数不会从调用它的函数中继承this. 如果嵌套函数作为对象的方法调用，那么其this指向调用该方法的对象； 如果嵌套函数作为普通函数调用，那么在非严格模式下，指向`window`，在严格模式是`undefined `
...
函数的`length`属性返回函数**预期传入的参数个数**，即函数**定义之中的参数个数**。 
