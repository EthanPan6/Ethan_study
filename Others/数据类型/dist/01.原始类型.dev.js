...
*/
//1. number类型
var iNum = 12; //2. string类型

var sWord = "hello"; //3. boolean类型

var isOk = true; //4. null，表示"没有对象"，即该处不应该有值。
//典型用法是：
// （1） 作为函数的参数，表示该函数的参数不是对象。
// （2） 作为对象原型链的终点。

var timer1 = null; //清除定时器
...
var unknown; //6. symbol  表示独一无二的值--------ES6
// 定义对象的唯一属性名。ES5的对象属性名都是字符串，容易造成属性名的冲突。

var unique0 = Symbol();
var unique1 = Symbol("unique");
console.log(unique0, unique1); // Symbol() Symbol(unique)

console.log(_typeof(unique0), _typeof(unique1)); //symbol symbol
